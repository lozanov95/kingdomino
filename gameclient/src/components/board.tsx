...
export const Board = memo(
    function Board({ board, handleOnClick }: { board: Domino[][] | null, handleOnClick: MouseEventHandler }) {

        return (
            <div className="board center" >
                {board?.map((el, idx) => {
                    return <Row key={idx} id={idx.toString()} elements={el} onClick={handleOnClick} />
                })}
            </div>
        )
    }
...
export function Row({ elements, id, onClick }: { id?: string, elements: Domino[] | null, onClick?: MouseEventHandler }) {
    return (
        <div className="row" id={id}>
            {elements?.map(({ name, nobles }, idx) => {
                return <BoardCell id={idx.toString()} key={idx} nobles={nobles} name={name} onClick={onClick} />
            })}
        </div>
    )
}
